...
| `/goal continue-node [goal-ref] <node-id>` | Keep the selected/current subagent session/worktree for a blocked/failed node, reset its controller retry counters, and ask the controller to continue in-place. |
| `/goal continue-subagent [goal-ref] <subagent-id>` | Continue a specific blocked/failed subagent in-place without allocating a fresh node runner. |
