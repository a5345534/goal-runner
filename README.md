...
| `/goal retry-node [goal-ref] <node-id>` | Reset a blocked/failed DAG node to `planned`, retire its non-terminal runner attempt, and resume controller scheduling so the node can start again. |
