...
export function formatGoalListSummary(goal) {
    ...
    if (approvedOpenSpecChange?.[1])
        return approvedOpenSpecChange[1];
    const explicitChange = summary.match(/^Implement the ([a-z0-9][a-z0-9-]+) change\b/i);
    if (explicitChange?.[1])
        return explicitChange[1];
