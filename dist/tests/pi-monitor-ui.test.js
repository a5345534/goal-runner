...
function parseExecutionPlanRow(line, header) {
    const columns = line
        .replace(/\x1b\[[0-9;]*m/g, "")
        .trim()
        .split(/\s{2,}/);
...
test("goal monitor execution plan uses aligned wide-row headers and status-row icons", () => {
    ...
    const controller = new GoalMonitorController(summary("active"), () => ({ lines: ["controller-tail"], entryCount: 1, messageCount: 1 }), () => ({
        nodes,
        subagents: [
            subagent({
...
test("goal monitor offers retry-node operation on blocked node rows", () => {
    ...
    const controller = new GoalMonitorController(summary("blocked"), () => ({ lines: ["controller-tail"], entryCount: 1, messageCount: 1 }), () => ({ nodes, subagents, refreshedAt: now.toISOString() }), () => now);
    controller.render(140, theme);
...
test("goal monitor live scroll remains available from controller scope", () => {
    ...
    assert.match(initial, /ops: \[nodes\]/);
...
test("Pi: full subagent ID only in selected detail / runner scope", () => {
    ...
    assert.match(runnerRendered, /ops: \[view\]/);
});
...
test("Pi: actions display user-facing labels but return existing operation IDs", () => {
    ...
    // The row actions are user-facing.
    assert.match(rendered, /ops: \[nodes\]/);
