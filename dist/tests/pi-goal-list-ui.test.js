...
import { GoalListController, filterGoals, sortGoals, formatGoalListState, formatGoalListMetrics, formatGoalListWhere, formatGoalListSummary, formatGoalListRow, } from "../adapters/pi/goal-list-ui.js";
...
test("formatGoalListState never renders duplicated status/activity pairs", () => {
    const states = ["active", "paused", "blocked", "budgetLimited", "usageLimited", "complete"];
...
test("formatGoalListWhere summarizes long branch/ref to last segment", () => {
    ...
    assert.ok(result.length < "goal/0e1bdb70-9fc4-40aa-b114-3b21dd4eddab/add-goal-list-regression-tests".length, `where "${result}" must be shorter than raw branch`);
});
test("formatGoalListWhere uses ref when branch is absent", () => {
    ...
    const result = formatGoalListWhere(goal);
    assert.ok(result === "@my-change" || result.endsWith("my-change"), `expected ref-derived where, got "${result}"`);
});
...
test("formatGoalListRow at 120 columns preserves meaningful change name visibility", () => {
    ...
    assert.ok(row.includes("improve-goal-"), `row "${row}" must contain meaningful change name prefix`);
    assert.ok(!row.toLowerCase().includes("implement the approved openspec change"), `row "${row}" MUST NOT contain boilerplate`);
...
test("formatGoalListRow result never exceeds requested visible width", () => {
    ...
    for (const width of [40, 60, 80, 100, 120]) {
        ...
        assert.ok(!row.toLowerCase().includes("implement the approved openspec change"), `row at width=${width} must not contain boilerplate`);
    }
});
...
test("compact row: render produces compact rows without raw boilerplate or duplicated state", () => {
    ...
    for (const width of [80, 120]) {
        ...
        for (const line of lines) {
            ...
            assert.ok(!line.includes("implement the approved openspec change"), `line must not contain boilerplate (width=${width})`);
            assert.ok(!line.includes("idle-eligible"), `line must not contain raw activity state (width=${width})`);
        }
    }
});
