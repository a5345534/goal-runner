...
test("debug report detects exhausted candidate chain anomalies", async () => {
    ...
    await runtime.saveGoalDagNode({
        ...
        modelResolution: {
            ...
            status: "blocked",
            attemptedCandidates: [
                { candidateIndex: 0, model: "model-a", compliance: { satisfiesMinimum: true, downgraded: false, missingCapabilities: [] }, status: "succeeded" },
                { candidateIndex: 1, model: "model-b", compliance: { satisfiesMinimum: true, downgraded: false, missingCapabilities: [] }, status: "succeeded" },
            ],
            switchEvents: [
