...
import { readOpencodeSessionMessages, readOpencodeTokenUsage, summariseOpencodeSession, buildOpencodeCompletionEvidence, } from "../adapters/opencode/index.js";
function makeClient(messages) {
    return {
        session: {
            messages: async ({ sessionID }) => ({ data: messages, sessionID }),
        },
    };
}
...
test("readOpencodeTokenUsage returns no usage when no assistant tokens exist", () => {
    const messages = [baseMessage({ role: "user" })];
    const usage = readOpencodeTokenUsage(messages);
...
test("buildOpencodeCompletionEvidence collects commands, signals, and tool names", () => {
    const messages = [
        ...
        baseMessage({
            id: "a2",
            role: "assistant",
            parts: [{ type: "tool", tool: "bash", args: { command: "npm test" } }],
        }),
        baseMessage({
            id: "a3",
            role: "assistant",
...
//# sourceMappingURL=opencode-session-transcript.test.js.map