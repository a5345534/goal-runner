...
import { buildGoalMonitorRuntimeSummary, deriveMonitorHealth, SESSION_STATE_LABELS, HIDDEN_CONTINUATION_STATE_LABELS, CONTROLLER_POLL_STATE_LABELS, } from "../adapters/pi/monitor-ui.js";
...
test("OpenCode RUNTIME section shows suppressed continuation as not-failure", () => {
    ...
    const lines = renderOpencodeMonitorLines(makeSummary(), { nodes: [makeNode({ status: "running" })], subagents: [makeSubagent({ status: "running" })] }, { now: () => NOW, ledgerEvents: events });
