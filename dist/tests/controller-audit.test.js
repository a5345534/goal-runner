...
import { isAuditDue, buildControllerAuditSnapshot, validateControllerAuditDecision, applyAuditActions, recordAuditActionEvents, formatAuditSummary, DEFAULT_AUDIT_INTERVAL_MS, DEFAULT_MAX_RECENT_EVENTS, DEFAULT_MAX_RECENT_VALIDATION_RESULTS, GOAL_CONTROLLER_AUDIT_LEDGER_EVENT_TYPES, } from "../core/controller-audit.js";
// ---------------------------------------------------------------------------
// Test helpers
// ---------------------------------------------------------------------------
 ...
 */
function makeCriticalRetryLoopDecision() {
    return makeValidDecision({
        risk: "critical",
        summary: "Node node-1 is stuck in a retry loop: 5 retries in 10 minutes with no progress.",
        findings: [
            {
                kind: "retry-loop",
                nodeId: "node-1",
 ...
 */
function makeCriticalCostSpikeDecision() {
    return makeValidDecision({
        risk: "critical",
 ...
 */
function makeHealthyNoopDecision() {
    return makeValidDecision({
        risk: "low",
...
test("isAuditDue returns false when audit is disabled", () => {
    ...
    assert.equal(isAuditDue(opts, "2026-06-15T11:00:00.000Z", new Date(now)), false);
});
test("isAuditDue returns true when enabled and no previous audit has run", () => {
    ...
    assert.equal(isAuditDue(opts, null, new Date(now)), true);
});
test("isAuditDue returns true when the interval has elapsed", () => {
    ...
    assert.equal(isAuditDue(opts, "2026-06-15T11:58:00.000Z", new Date(now)), true);
});
test("isAuditDue returns false when the interval has not elapsed", () => {
    const opts = defaultOptions({ intervalMs: 60_000 }); // 1 minute
    // Last audit was 10 seconds ago
    assert.equal(isAuditDue(opts, "2026-06-15T11:59:50.000Z", new Date(now)), false);
});
test("isAuditDue returns true exactly at the interval boundary", () => {
    ...
    assert.equal(isAuditDue(opts, "2026-06-15T11:59:00.000Z", new Date(now)), true);
});
test("isAuditDue handles Date object for lastAuditAt", () => {
    const opts = defaultOptions({ intervalMs: 60_000 });
    // 2 minutes ago via Date object
    assert.equal(isAuditDue(opts, new Date("2026-06-15T11:58:00.000Z"), new Date(now)), true);
});
test("isAuditDue returns true when lastAuditAt is an invalid date string", () => {
    const opts = defaultOptions({ intervalMs: 60_000 });
    assert.equal(isAuditDue(opts, "not-a-date", new Date(now)), true);
});
test("isAuditDue respects the default interval when intervalMs is not set", () => {
    ...
    assert.equal(isAuditDue(opts, now, new Date(now)), false);
    ...
    assert.equal(isAuditDue(opts, thirtyOneMinAgo, nowDate), true);
});
...
test("buildControllerAuditSnapshot respects maxRecentEvents", () => {
    ...
    assert.equal(snapshot.recentControllerEvents[snapshot.recentControllerEvents.length - 1]
        .type, "node.complete");
});
...
test("validateControllerAuditDecision accepts a decision with all valid risk levels", () => {
    for (const risk of ["low", "medium", "high", "critical"]) {
        const result = validateControllerAuditDecision(makeValidDecision({ risk }));
...
test("validateControllerAuditDecision accepts a decision with all valid finding kinds", () => {
    ...
    for (const kind of kinds) {
        ...
        const result = validateControllerAuditDecision(decision);
        assert.equal(result.valid, true, `should accept finding kind=${kind}`);
    }
});
test("validateControllerAuditDecision accepts a decision with all valid action types", () => {
    ...
    for (const action of actions) {
        ...
        const result = validateControllerAuditDecision(decision);
        assert.equal(result.valid, true, `should accept action=${action}`);
    }
});
...
test("validateControllerAuditDecision rejects invalid risk value", () => {
    const decision = makeValidDecision({ risk: "super-bad" });
    const result = validateControllerAuditDecision(decision);
    assert.equal(result.valid, false);
    if (!result.valid) {
...
test("validateControllerAuditDecision rejects missing summary", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("summary must be a non-empty")));
    }
});
...
test("validateControllerAuditDecision rejects invalid finding kind", () => {
    const decision = makeValidDecision({
        findings: [{ kind: "invented-kind", evidence: [], confidence: "low" }],
    });
    const result = validateControllerAuditDecision(decision);
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("findings[0].kind must be one of")));
    }
});
test("validateControllerAuditDecision rejects invalid finding confidence", () => {
    const decision = makeValidDecision({
        findings: [{ kind: "no-progress", evidence: [], confidence: "unknown" }],
    });
    const result = validateControllerAuditDecision(decision);
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("findings[0].confidence must be one of")));
    }
});
test("validateControllerAuditDecision rejects finding with non-array evidence", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("findings[0].evidence must be an array")));
    }
});
test("validateControllerAuditDecision rejects evidence entries that are not strings", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("findings[0].evidence entries must be strings")));
    }
});
test("validateControllerAuditDecision rejects incorrect nodeId type in finding", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("findings[0].nodeId must be a string")));
    }
});
test("validateControllerAuditDecision rejects incorrect subagentId type in finding", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("findings[0].subagentId must be a string")));
    }
});
test("validateControllerAuditDecision rejects recommendedActions not an array", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("recommendedActions must be an array")));
    }
});
test("validateControllerAuditDecision rejects invalid action type", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("recommendedActions[0].action must be one of")));
    }
});
test("validateControllerAuditDecision rejects missing reason in action", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("recommendedActions[0].reason must be a non-empty")));
    }
});
test("validateControllerAuditDecision rejects missing requiresUserApproval", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("recommendedActions[0].requiresUserApproval must be a boolean")));
    }
});
test("validateControllerAuditDecision rejects incorrect nodeId type in action", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("recommendedActions[0].nodeId must be a string")));
    }
});
test("validateControllerAuditDecision rejects incorrect subagentId type in action", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("recommendedActions[0].subagentId must be a string")));
    }
});
...
test("validateControllerAuditDecision accepts a finding that is null inside findings array", () => {
    ...
    assert.equal(result.valid, false);
    if (!result.valid) {
        assert.ok(result.errors.some((e) => e.includes("findings[0] is not an object")));
    }
});
// ---------------------------------------------------------------------------
...
test("applyAuditActions does NOT pause on medium risk even with high confidence", () => {
    ...
    assert.ok(result.skipped[0].reason.includes('risk is "medium"'));
});
test("applyAuditActions does NOT pause on critical risk with low confidence", () => {
    ...
    assert.ok(result.skipped.some((s) => s.reason.includes("no finding has high confidence")));
});
test("applyAuditActions does NOT pause when pauseOnCritical is disabled", () => {
    ...
    assert.equal(result.applied.length, 0);
    assert.equal(result.skipped.length, 1);
    assert.ok(result.skipped[0].reason.includes("pauseOnCritical is disabled"));
});
test("applyAuditActions skips non-pause actions (mark-node-blocked, cap-retries, etc.)", () => {
    ...
    for (const skip of result.skipped) {
        assert.ok(skip.reason.includes("deterministic confirmation"), `skip reason for ${skip.action.action}: ${skip.reason}`);
    }
});
...
test("recordAuditActionEvents records pause-goal events correctly", async () => {
    ...
    assert.ok(pausedEvents[0].details.appliedActions.includes("pause-goal"));
    assert.ok(pausedEvents[0].details.findingKinds.includes("retry-loop"));
});
...
test("recordAuditActionEvents skips goal_paused when shouldPauseGoal is false", async () => {
    ...
    await recordAuditActionEvents(result, decision, recorder, now);
    assert.equal(events.filter((e) => e.type === "goal_paused_by_controller_audit").length, 0);
    assert.equal(events.filter((e) => e.type === "controller_audit_action_skipped").length, 1);
});
// ---------------------------------------------------------------------------
// 6. Scenario tests
// ---------------------------------------------------------------------------
test("scenario: retry-loop triggers pause (spec: Critical high-confidence retry-loop triggers auto pause)", () => {
    ...
    const events = [
        // 5 retry failures
        ...Array.from({ length: 5 }, (_, i) => makeEvent({
            eventId: `retry-${i}`,
            type: "continuation_retryable_failure",
    ...
    const decision = {
        risk: "critical",
        summary: "Node looping-node is stuck in a retry loop: 5 retries in window with no progress.",
        findings: [
            {
                kind: "retry-loop",
                nodeId: "looping-node",
    ...
    // Apply action policy
    const result = applyAuditActions(validation.valid ? validation.decision : decision, defaultOptions({ pauseOnCritical: true }));
    assert.equal(result.shouldPauseGoal, true);
    assert.equal(result.applied.length, 1);
    assert.equal(result.applied[0].action.action, "pause-goal");
...
test("scenario: cost-spike triggers pause (spec: Critical cost-spike triggers auto pause)", () => {
    ...
    assert.equal(snapshot.costSignals.estimatedCostLastWindow, Math.round((55000 / 1000) * 0.01 * 100) / 100);
    assert.equal(snapshot.goal.tokensUsed, 55000);
    // Simulate audit model decision for cost-spike
    const decision = {
        risk: "critical",
        summary: "Token usage spiked to 55K in the last window; estimated cost $0.55.",
        findings: [
            {
                kind: "cost-spike",
                evidence: [
...
test("scenario: healthy progress returns noop (spec: Steady progress returns noop)", () => {
    ...
    const validation = validateControllerAuditDecision(decision);
    assert.equal(validation.valid, true);
    const result = applyAuditActions(validation.valid ? validation.decision : decision, defaultOptions());
    assert.equal(result.shouldPauseGoal, false);
    assert.equal(result.applied.length, 0);
    assert.equal(result.skipped.length, 0);
});
test("scenario: invalid audit output is detected and produces no side effects", () => {
    ...
    if (!result.valid) {
        ...
        assert.ok(errorText.includes("summary") || errorText.includes("findings"), "should report summary or findings error");
    }
...
test("GOAL_CONTROLLER_AUDIT_LEDGER_EVENT_TYPES includes all required event types", () => {
    ...
    for (const t of requiredTypes) {
        assert.ok(GOAL_CONTROLLER_AUDIT_LEDGER_EVENT_TYPES.includes(t), `missing event type: ${t}`);
    }
});
// ---------------------------------------------------------------------------
// 9. Default constants
...
//# sourceMappingURL=controller-audit.test.js.map