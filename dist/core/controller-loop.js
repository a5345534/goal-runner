...
import { extractQuestionMarker, isQuestionPendingState } from "./subagent-adapter.js";
...
async function tryAutoRecoverFailedNode(runtime, adapter, node, subagent, state, result, options, tickStartedAt, observedError) {
    ...
    if (isModelSwitchable && hasCandidateChain(node)) {
        const switched = await tryCandidateSwitchForModelFailure(runtime, adapter, node, subagent, state, result, options, tickStartedAt, errorMessage, retryCount, maxRetries);
        if (switched)
            return true;
 ...
 */
async function triageSubagentQuestion(runtime, options, _state, node, subagent, result, tickStartedAt) {
...
async function persistQuestionTriage(runtime, subagent, outcome, node, _result, tickStartedAt) {
...
function extractFirstOptionId(body) {
 ...
 */
function findContextAnswer(question, node) {
 ...
 */
function isAnsweredByText(lowerQuestion, context) {
 ...
 */
function getActiveCandidateIndex(node) {
 ...
 * Defaults to 0.
 */
function getCandidateRetryCount(node) {
 ...
 */
function getAttemptsPerCandidate(node) {
...
function candidatePlanEntries(node) {
    const resolution = node.modelResolution ?? node.preparedResources?.modelResolution;
    if (resolution?.candidatePlan?.length)
        return resolution.candidatePlan;
 ...
 */
function getNextCandidateIndex(node) {
 ...
 */
function getCandidateModelForIndex(node, index) {
 ...
 * are NOT switchable.
 */
function isModelSwitchableFailure(message) {
 ...
 */
function withUpdatedCandidateState(node, candidateIndex, candidateRetryCount, modelArg, tickStartedAt) {
 ...
 */
async function recordCandidateEvent(runtime, goalId, eventType, details, at) {
 ...
 */
async function tryCandidateSwitchForModelFailure(runtime, adapter, node, subagent, state, result, options, tickStartedAt, errorMessage, retryCount, maxRetries) {
...
function withRuntimeCandidateSwitchEvidence(node, fromCandidateIndex, fromModel, toCandidateIndex, toModel, reason) {
    const resolution = node.modelResolution ?? node.preparedResources?.modelResolution;
    if (!resolution)
        return undefined;
    return {
 ...
 */
function buildCandidateRetryPrompt(node, subagent, errorMessage, candidateRetry, attemptsPerCandidate, candidateIndex, candidateModel) {
    return [
 ...
 */
function buildCandidateSwitchPrompt(node, errorMessage, nextCandidateIndex, nextModel, maxRetries) {
    return [
