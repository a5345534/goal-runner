...
export class JsonlGoalDebugTracer {
    enabled = true;
    traceFile;
    now;
    randomId;
    ...
    async trace(category, operation, context, action) {
...
export function createGoalDebugTracerFromEnv(options = {}) {
...
export function instrumentGoalStore(store, tracer) {
    if (!tracer?.enabled)
        return store;
    return new InstrumentedGoalStore(store, tracer);
}
class InstrumentedGoalStore {
    store;
    tracer;
    constructor(store, tracer) {
        this.store = store;
        this.tracer = tracer;
    }
    ...
    async saveGoalSessionMetadata(metadata) {
        return this.trace("saveGoalSessionMetadata", {
    ...
    async trace(operation, context, action, summarize) {
        const started = Date.now();
        try {
            const value = await action();
            this.tracer.record({
...
export function formatGoalDebugReport(report) {
    const lines = [
    ...
    if (report.goal.executionWorkspace)
        lines.push(`Workspace: ${report.goal.executionWorkspace}`);
    if (report.goal.sessionFile)
        lines.push(`Session: ${report.goal.sessionFile}`);
    lines.push("");
...
export function recordGoalDebugSnapshot(tracer, input) {
    if (!tracer?.enabled)
        return;
...
function subagentContext(subagent) {
    return {
...
function summarizeGoalRecord(goal) {
...
function summarizeSubagent(subagent) {
...
function summarizeSubagents(subagents) {
...
function normalizeGoalDebugGoal(goal) {
...
function runnerLaunchFailureCount(node) {
...
function isNonTerminalSubagentStatus(status) {
...
function summarizeLedgerEvent(event) {
    ...
    if (typeof reason === "string")
        return truncate(reason, 160);
    const error = details.error;
    if (typeof error === "string")
        return truncate(error, 160);
    return undefined;
}
...
function sanitizeValue(value, depth, key = "") {
    ...
    if (Array.isArray(value))
        return value.slice(0, 30).map((item) => sanitizeValue(item, depth + 1, key));
