...
import { qualityProfilesOf } from "./quality-profiles.js";
...
const REVIEW_AUDIT_PROFILES = new Set([
...
function evaluateQualityProfileGates(request, result, state) {
...
function evaluateSingleProfileGate(profile, request, result, state) {
    switch (profile) {
        ...
        case "docs-adr-required": {
            ...
            const hasDocOutput = request.node.expectedOutputs.some((output) => /\.(?:md|adoc|rst|txt)$/i.test(output) ||
                /docs?\//.test(output) ||
                /adr\//.test(output) ||
 ...
 */
function checkDependentReviewNodesComplete(request, state) {
    ...
    const dependentNodes = state.nodes.filter((node) => node.dependencyNodeIds.includes(request.node.nodeId) &&
        (node.kind === "review" || node.kind === "audit" || node.kind === "validation"));
    if (dependentNodes.length === 0)
        return false;
    return dependentNodes.some((node) => node.status === "complete" || node.status === "superseded");
}
...
function buildValidationSignals(request, result) {
    ...
    if (qualityProfilePolicyConfigured(request) && !hasQualityProfilePolicyFailure(result))
        signals.push(`quality profile gates passed: ${qualityProfilesOf(request.node).join(", ")}`);
    if (signals.length === 0)
...
function qualityProfilePolicyConfigured(request) {
    return qualityProfilesOf(request.node).length > 0;
}
function hasQualityProfilePolicyFailure(result) {
