...
import {
  type ChoiceActionChoice,
  type ChoiceActionInputMode,
  type ChoiceActionRenderMode,
  type ChoiceActionRequest,
  type ChoiceActionSelectionResult,
...
export type {
  ChoiceActionChoice,
  ChoiceActionInputMode,
  ChoiceActionRenderMode,
  ChoiceActionRequest,
 ...
 */
export function renderChoiceActionTextFallback(request: ChoiceActionRequest): string {
  const lines: string[] = [];
...
// ---------------------------------------------------------------------------
// Input normalization (text input → ChoiceActionSelectionResult)
// ---------------------------------------------------------------------------
 ...
 */
export function normalizeChoiceActionTextInput(
  request: ChoiceActionRequest,
  userInput: string,
  selectedAt?: string,
): ChoiceActionNormalizationResult {
  ...
  if (byId) {
    return {
      result: buildChoiceActionSelectionResult(
        request,
        byId,
        "canonical_text",
        "text_fallback",
  ...
  if (request.allowTextAliases) {
    ...
    const byAlias = request.choices.find((c) =>
      c.aliases.some((a) => normalizeAlias(a) === normalizedInput),
    );
    if (byAlias) {
      return {
        result: buildChoiceActionSelectionResult(
 ...
 */
export function buildInteractiveChoiceActionResult(
  request: ChoiceActionRequest,
  choice: ChoiceActionChoice,
  selectedAt?: string,
): ChoiceActionSelectionResult {
  const result = buildChoiceActionSelectionResult(
    request,
    choice,
    "interactive",
    "interactive",
 ...
 * explicit default).
 */
export function buildDefaultedChoiceActionResult(
  request: ChoiceActionRequest,
  selectedAt?: string,
): ChoiceActionSelectionResult | undefined {
  if (!request.defaultChoiceId) return undefined;
  ...
  const result = buildChoiceActionSelectionResult(
    ...
    choice,
    "defaulted",
    request.allowTextAliases ? "text_fallback" : "text_fallback",
    selectedAt,
  );
  ...
  try {
    ...
    if (!request.allowDisabledOverride) return undefined;
...
function buildChoiceActionSelectionResult(
  request: ChoiceActionRequest,
  choice: ChoiceActionChoice,
