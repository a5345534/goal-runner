...
export function normalizePiModelArg(modelArg: string | undefined): string | undefined {
  ...
  if (dot <= 0 || dot === base.length - 1) return trimmed;

  const provider = base.slice(0, dot);
  const model = base.slice(dot + 1);
  if (!isLikelyPiProviderPrefix(provider)) return trimmed;
  return `${provider}/${model}${suffix}`;
}

function isLikelyPiProviderPrefix(provider: string): boolean {
  if (KNOWN_PI_MODEL_PROVIDERS.has(provider)) return true;
