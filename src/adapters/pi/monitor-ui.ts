...
export type GoalMonitorRunnerOperation = "openSession" | "continueSubagent" | "stop" | "kill" | "archive";

export type GoalMonitorSelection =
  | { kind: "action"; action: GoalMonitorAction }
  | { kind: "nodeOperation"; operation: GoalMonitorNodeOperation; nodeId: string }
...
export type MonitorHiddenContinuationState =
  | "eligible"
  | "suppressed"
  | "reserved"
  | "started"
...
export type MonitorControllerPollState =
  | "active"
  | "leased"
  | "skipped"
  | "stopped"
...
export const HIDDEN_CONTINUATION_STATE_LABELS: Record<MonitorHiddenContinuationState, string> = {
  eligible: "ELIGIBLE",
...
export const CONTROLLER_POLL_STATE_LABELS: Record<MonitorControllerPollState, string> = {
  active: "ACTIVE",
  leased: "LEASED",
  skipped: "SKIPPED",
  stopped: "STOPPED",
 ...
 */
export function buildGoalMonitorRuntimeSummary(
  goal: GoalSummary,
  subagents: GoalSubagentRecord[],
  options: BuildRuntimeSummaryOptions = {},
): GoalMonitorRuntimeSummary {
...
function renderAlignedColumns(values: string[], widths: number[]): string {
  return values
    .map((value, index) => {
...
function deriveSessionState(
...
): GoalMonitorRuntimeSummary["session"] {
  if (harness) {
    if (harness.activeTurnId) return { state: "active-turn", activeTurnId: harness.activeTurnId };
    if (harness.materialized) return { state: "idle" };
  ...
  if (activity.includes("active-turn")) return { state: "active-turn" };
  if (activity.includes("idle")) return { state: "idle" };
  if (["complete"].includes(goal.status)) return { state: "not-materialized" };
  if (!goal.sessionFile || !existsSync(goal.sessionFile)) return { state: "missing" };
  return { state: "unknown" };
}

function deriveHiddenContinuationState(
  goal: GoalSummary,
  harness?: HarnessState,
  reservation?: ContinuationReservation,
): GoalMonitorRuntimeSummary["hiddenContinuation"] {
  ...
  const activity = goal.activityState ?? "";
  if (activity.includes("suppressed")) return { state: "suppressed", reason: "suppressed by runtime" };
  if (activity.includes("eligible") || activity.includes("idle-eligible")) return { state: "eligible" };
  if (activity.includes("idle")) return { state: "eligible" };
...
function deriveControllerPollState(
  ledgerEvents: GoalLedgerEvent[],
  pollGraceMs: number,
): GoalMonitorRuntimeSummary["controllerPoll"] {
  ...
  const details = (lastEvent.details ?? {}) as Record<string, unknown>;
  const eventName = details.event as string;
...
function deriveRunnerCounts(
  subagents: GoalSubagentRecord[],
  runners: PiBackgroundRunnerRecord[],
): GoalMonitorRuntimeSummary["runners"] {
  const runnerAlive = new Set(
    runners.filter((r) => r.runnerAlive || r.childAlive).map((r) => r.subagentId ?? r.configPath),
  );

  // Map by subagentId for quick look-up.
  const runnerBySubagent = new Map<string, PiBackgroundRunnerRecord[]>();
  ...
  for (const subagent of subagents) {
    ...
    if (aliveCount > 0) {
      ...
      if (totalCount > aliveCount) duplicateStopped += totalCount - aliveCount;
...
type GoalMonitorRowOperation =
  ...
  | { kind: "node"; operation: GoalMonitorNodeOperation; nodeId: string; label: string }
...
export class GoalMonitorController {
  ...
  render(width: number, theme: GoalListThemeLike): string[] {
    ...
    if (liveDiag) lines.push(truncateToWidth(theme.fg("warning", liveDiag), width));
    if (liveLines.length === 0) lines.push(truncateToWidth(theme.fg("muted", "No live entries available"), width));
    ...
    for (const line of liveLines.slice(liveStart, liveEnd)) lines.push(truncateToWidth(line, width));
...
function operationsForListItem(item: GoalMonitorListItem | undefined, goal: GoalSummary, dag: GoalMonitorDagSnapshot): GoalMonitorRowOperation[] {
  ...
  if (item.kind === "controller") {
    return [
      ...
      ...controllerActions(goal).map((action): GoalMonitorRowOperation => ({ kind: "action", action, label: userActionLabel(action) })),
    ];
  }
  if (item.kind === "node") {
    ...
    const operations: GoalMonitorRowOperation[] = [
  ...
  const operations: GoalMonitorRowOperation[] = [{ kind: "internal", operation: "view", label: userActionLabel("view") }];
  if (subagent?.sessionFile) operations.push({ kind: "runner", operation: "openSession", subagentId: item.subagentId, label: userActionLabel("openSession") });
  if (node && subagent && canContinueSubagentInPlace(node, subagent)) operations.push({ kind: "runner", operation: "continueSubagent", subagentId: item.subagentId, label: userActionLabel("continueSubagent") });
  if (hasLiveRunner) operations.push({ kind: "runner", operation: "stop", subagentId: item.subagentId, label: userActionLabel("stop") });
...
function canContinueSubagentInPlace(node: GoalDagNode, subagent: GoalSubagentRecord): boolean {
  return canRetryNode(node) && subagent.status !== "complete" && Boolean(subagent.sessionFile);
}

/** Map an operation ID to its user-facing label using ACTION_DISPLAY_LABELS. */
function userActionLabel(operationId: string): string {
...
function formatPlainOperation(operation: GoalMonitorRowOperation | undefined): string {
  ...
  if (operation.kind === "internal") return operation.operation;
  if (operation.kind === "action") return operation.action;
  if (operation.kind === "node") return operation.operation;
  if (operation.kind === "runner") return operation.operation;
  return "-";
}
...
function formatQuestionOutcome(outcome: GoalSubagentQuestionOutcome): string {
  const triageLabels: Record<GoalSubagentQuestionOutcome["triageKind"], string> = {
...
function renderOverviewHeader(
  overview: ReturnType<typeof buildGoalMonitorOverview>,
  _runtimeSummary: GoalMonitorRuntimeSummary,
  width: number,
  isNarrow: boolean,
  theme: GoalListThemeLike,
): string[] {
  ...
  const healthColor =
    overview.health === "OK" || overview.health === "Complete" ? "success" :
  ...
  if (isNarrow) {
    ...
    lines.push(truncateToWidth(
      `${theme.fg(healthColor, `Health: ${healthLabel}`)}${problemPart ? theme.fg("warning", problemPart) : ""}`,
      width,
    ));
    lines.push(truncateToWidth(theme.fg("dim", `Progress: ${overview.progressLabel}`), width));
    lines.push(truncateToWidth(theme.fg("dim", `Runtime: ${overview.runtimeLabel}`), width));
...
function renderExecutionPlanSection(
  ...
  if (wideColumns) {
    lines.push(truncateToWidth(
      theme.fg("dim", renderAlignedColumns([
        "ICON",
        "NODE ID",
        "STATUS",
        "MODEL",
...
function buildExecutionPlanWideColumns(width: number): number[] | undefined {
  const iconWidth = 4;
...
function truncateNarrow(text: string, maxLen: number): string {
  if (text.length <= maxLen) return text;
  // Try to keep node slug and status by removing less important fields.
  const parts = text.split(" · ");
  if (parts.length <= 1) return `${text.slice(0, maxLen - 3)}...`;
...
function formatRuntimeBandLines(
  summary: GoalMonitorRuntimeSummary,
  health: { health: MonitorHealth; nextAction: string },
  width: number,
  theme: GoalListThemeLike,
): string[] {
...
function formatRunnerSummary(runners: GoalMonitorRuntimeSummary["runners"]): string {
  const parts: string[] = [];
  if (runners.running > 0) parts.push(`${runners.running} running`);
  if (runners.stopped > 0) parts.push(`${runners.stopped} stopped`);
  if (runners.duplicateStopped > 0) parts.push(`${runners.duplicateStopped} dup`);
