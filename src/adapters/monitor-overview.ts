...
// ── Re-export the existing runtime summary builder and its types ──
export type {
  GoalMonitorRuntimeSummary,
  MonitorHealth,
} from "./pi/monitor-ui.js";

// ── Extended health taxonomy ──

export type ExtendedMonitorHealth =
  | MonitorHealth
  | "Complete"
  | "Complete with warnings"
  | "Running";

export const EXTENDED_MONITOR_HEALTH_LABELS: Record<ExtendedMonitorHealth, string> = {
  OK: "OK",
...
const RUNTIME_SESSION_USER_LABELS: Record<string, string> = {
...
const RUNTIME_HIDDEN_USER_LABELS: Record<string, string> = {
...
const RUNTIME_POLL_USER_LABELS: Record<string, string> = {
  active: "polling",
  leased: "leased",
  skipped: "skipped",
  stopped: "stopped",
 ...
 */
export function buildGoalMonitorOverview(
  goal: GoalSummary,
  dag: { nodes: GoalDagNode[]; subagents: GoalSubagentRecord[]; ledgerEvents?: GoalLedgerEvent[] },
  runtimeSummary: GoalMonitorRuntimeSummary,
  options: BuildGoalMonitorOverviewOptions = {},
): GoalMonitorOverview {
...
function isNewerReplacement(
  failed: GoalSubagentRecord,
  other: GoalSubagentRecord,
): boolean {
  if (other.nodeId !== failed.nodeId) return false;
  if (other.subagentId === failed.subagentId) return false;
  if (!["running", "complete"].includes(other.status)) return false;

  const failedTime = Date.parse(failed.updatedAt ?? failed.createdAt);
  const otherTime = Date.parse(other.updatedAt ?? other.createdAt);
  if (!Number.isFinite(failedTime) || !Number.isFinite(otherTime)) return false;
...
function isUnresolvedResidualIssue(
  ...
  return !allSubagents.some(
    (other) => isNewerReplacement(subagent, other),
  );
}
 ...
 */
export function summarizeMonitorProblem(
  goal: GoalSummary,
  nodes: GoalDagNode[],
  subagents: GoalSubagentRecord[],
): string {
  ...
  // Goal-level issues.
  if (goal.status === "blocked") return "goal blocked · inspect for details";
  if (goal.status === "paused") return "goal paused · resume to continue";

  // No obvious problem.
  const hasRunning = subagents.some((s) => s.status === "running");
  if (hasRunning) return "none · running";

  const allComplete = subagents.length > 0 && subagents.every((s) => s.status === "complete");
  if (allComplete) return "none · awaiting finalisation";

  if (goal.status === "active") return "none · active";
 ...
 */
export function formatRuntimeSummaryForOverview(
  summary: GoalMonitorRuntimeSummary,
): string {
...
function formatRunnerCountLabel(runners: GoalMonitorRuntimeSummary["runners"]): string {
  const parts: string[] = [];
...
function isMeaningfulEvent(event: GoalLedgerEvent): boolean {
...
function formatEventLine(event: GoalLedgerEvent): string {
...
function formatEventSummary(eventName: string, details: Record<string, unknown>): string {
  if (typeof details.objective === "string") return truncateReason(details.objective, 80);
  if (typeof details.summary === "string" && details.summary) return truncateReason(details.summary, 80);
  if (typeof details.reason === "string" && details.reason) return truncateReason(details.reason, 80);
  if (typeof details.error === "string" && details.error) return truncateReason(details.error, 80);
  if (typeof details.status === "string") return details.status;
  if (typeof details.action === "string") return details.action;
  if (typeof details.to === "string") return `→ ${details.to}`;
  return "";
}
...
function truncateSlug(slug: string, maxLen: number): string {
  ...
  if (maxLen <= 4) return slug.slice(0, maxLen);
...
function truncateReason(reason: string, maxLen: number): string {
  ...
  if (maxLen <= 4) return cleaned.slice(0, maxLen);
  return `${cleaned.slice(0, maxLen - 3)}...`;
}

function findLatestSubagent(
  _nodes: GoalDagNode[],
  subagents: GoalSubagentRecord[],
  nodeId: string,
...
function formatProgressLabel(
  ...
  if (nodes.length === 0) return "no DAG nodes planned";
  ...
  const parts: string[] = [];
  if (complete > 0) parts.push(`${complete}/${nodes.length} nodes complete`);
  if (running > 0) parts.push(`${running} running`);
  if (blocked > 0) parts.push(`${blocked} blocked`);
  if (planned > 0 && running === 0 && blocked === 0) parts.push(`${planned} planned`);
...
function formatNextActionLabel(
  ...
  if (health === "Complete") return "goal complete · archive or inspect";
  if (health === "Complete with warnings") return "inspect residual issues in execution plan";
  if (health === "Blocked") return `inspect blocked items · ${problem}`;
  if (health === "Needs attention") return `inspect blocked items · ${problem}`;
  if (health === "Waiting") return goal.status === "paused" ? "resume goal to continue" : "goal waiting on budget/usage limit reset";
  if (health === "Stalled") return "no running subagents · check controller poll and auto-continue";
  if (health === "Running") return "monitor progress";
