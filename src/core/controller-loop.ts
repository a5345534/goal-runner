...
function isRecoverableRunnerLaunchError(message: string): boolean {
  return isMissingSessionTerminalError(message) || isTerminatedSessionError(message) || isTransientError(message);
}

function runnerLaunchFailureCount(resources: GoalNodePreparedResources | undefined): number {
  const value = resources?.metadata?.runnerLaunchFailureCount;
  return typeof value === "number" && Number.isFinite(value) && value > 0 ? Math.floor(value) : 0;
}

function withRunnerLaunchFailureMetadata(resources: GoalNodePreparedResources | undefined, errorMessage: string, attempt: number, tickStartedAt: string): GoalNodePreparedResources | undefined {
...
async function keepRunnerLaunchFailureRecoverable(
  runtime: GoalControllerRuntimePort,
  node: GoalDagNode,
  errorMessage: string,
  ...
  if (!isRecoverableRunnerLaunchError(errorMessage) || attempt >= maxAttempts) return false;
  ...
  for (const node of state.nodes) {
    ...
    try {
      ...
      const recoverableLaunchFailure = await keepRunnerLaunchFailureRecoverable(
        runtime,
        { ...node, preparedResources },
        `stale runnerStarting restart failed: ${errorMessage}`,
        result,
        tickStartedAt,
        "staleRunnerStarting.restartFailedRecoverable",
        options.maxAutoRetries,
        { subagentId: preparedResources.subagentId },
      );
      if (recoverableLaunchFailure) continue;
...
function buildIntegrationSubagentPatch(
  subagent: GoalSubagentRecord,
  integration: GoalControllerIntegrationResult,
  tickStartedAt: string,
): Partial<GoalSubagentRecord> {
  return {
...
function nodeHasDownstreamDependents(state: GoalOrchestrationState, node: GoalDagNode): boolean {
...
interface DependencyIntegrationEnsureResult {
  blockReason?: string;
  integrated: boolean;
}

async function ensureDependencyIntegrationsBeforeWorkspaceAllocation(
  runtime: GoalControllerRuntimePort,
  goalId: string,
  options: GoalControllerTickOptions,
  state: GoalOrchestrationState,
  node: GoalDagNode,
  tickStartedAt: string,
): Promise<DependencyIntegrationEnsureResult> {
  ...
  for (const dependencyId of node.dependencyNodeIds) {
    ...
    if (!upstreamSubagent) return { blockReason: `dependency ${dependencyId} has no subagent record`, integrated };
    if (requiredSubagentIntegrationTerminalSuccess(upstreamSubagent)) continue;
    if (!hasSubagentBranchOrWorkspaceEvidence(upstreamSubagent)) continue;
    if (!options.integrator) return { blockReason: `dependency ${dependencyId} requires integration but no controller integrator is configured`, integrated };

    const upstreamNode = state.nodes.find((candidate) => candidate.nodeId === dependencyId);
    if (!upstreamNode) return { blockReason: `dependency ${dependencyId} has no node record`, integrated };
    ...
    let integration: GoalControllerIntegrationResult;
    try {
...
async function startReadyNodes(
  ...
  for (const node of queue.ready) {
    ...
    if (upstreamIntegration.integrated) state = await runtime.getGoalOrchestrationState(goalId);
    ...
    try {
      ...
      const recoverableLaunchFailure = await keepRunnerLaunchFailureRecoverable(
        ...
        result,
        tickStartedAt,
        error instanceof ControllerActionTimeoutError ? "node.runnerLaunchTimedOut" : "node.runnerLaunchFailedRecoverable",
        options.maxAutoRetries,
        { timeout: error instanceof ControllerActionTimeoutError },
      );
      if (recoverableLaunchFailure) continue;
      const summary = isProviderLimitError(errorMessage)
        ? quotaBlockedSummary(errorMessage)
...
function compactDependencyBlockerDetail(detail: string): string {
  const firstLine = detail
    .split(/\r?\n/)
    .map((line) => line.trim())
    .find((line) => line && !/^at\s+/.test(line));
  const compact = (firstLine ?? detail).replace(/\s+/g, " ").trim();
  if (!compact) return "blocked";
  if (/^dependency blocked:/i.test(compact)) return "dependency blocked";
...
function latestSubagentForNode(subagents: GoalSubagentRecord[], nodeId: string): GoalSubagentRecord | undefined {
  let latest: GoalSubagentRecord | undefined;
  for (const subagent of subagents) {
    if (subagent.nodeId !== nodeId) continue;
