...
// ── Submodule target-branch publication policy types ──
 ...
 */
export const TRUSTED_SUBMODULE_TARGET_BRANCH_URL_PATTERNS_ENV =
  "AGENT_GOAL_NATIVE_GIT_TRUSTED_SUBMODULE_TARGET_BRANCH_URL_PATTERNS";
 ...
 */
export function resolveSubmoduleTargetBranchPolicy(
  policy: NativeGitSubmoduleTargetBranchPolicy,
  options: { env?: NodeJS.ProcessEnv } = {},
): NativeGitSubmoduleTargetBranchPolicy {
  const configuredPatterns = parseTrustedSubmoduleUrlPatterns(
    options.env?.[TRUSTED_SUBMODULE_TARGET_BRANCH_URL_PATTERNS_ENV],
  );
  return {
    ...policy,
...
export class NativeGitWorkspaceManager {
  ...
  integrateSubagentBranch(request: NativeGitSubagentBranchIntegrationRequest): NativeGitSubagentBranchIntegrationResult {
    ...
    if (preIntegrationCheckoutSync.status === "blocked") {
      return nativeGitIntegrationFailure(
        request,
        `controller submodule checkout sync failed before integration: ${preIntegrationCheckoutSync.summary}`,
        { sourceBranch, sourceRef, sourceHead },
        undefined,
        submoduleCheckoutSyncValidationSignals(preIntegrationCheckoutSync),
      );
    }
    ...
    try {
      ...
      if (postMergeCheckoutSync.status === "blocked") {
        ...
        return nativeGitIntegrationFailure(
          request,
          `submodule checkout sync failed after merge: ${postMergeCheckoutSync.summary}`,
          { sourceBranch, sourceRef, sourceHead },
          undefined,
          submoduleCheckoutSyncValidationSignals(postMergeCheckoutSync),
        );
      }
      ...
      if (!postMergeValidation.ok) {
        ...
        return nativeGitIntegrationFailure(
          ...
          { sourceBranch, sourceRef, sourceHead },
          buildPostMergeValidationFollowupPrompt(request, postMergeValidation.summary),
          [...preIntegrationCheckoutSyncSignals, ...postMergeCheckoutSyncSignals, ...postMergeValidation.validationSignals],
        );
      }
  ...
  ensureSubmoduleGitlinksDurablyPublished(request: NativeGitSubmodulePublishRequest): NativeGitSubmodulePublishResult {
    ...
    const allGitlinks: ChangedSubmoduleGitlink[] = [...gitlinks];
    const published: PublishedSubmoduleRef[] = [];
    const verified: VerifiedSubmoduleRef[] = [];
    const blockers: SubmodulePublishBlocker[] = [];

    const appendNestedResult = (nested: NestedSubmoduleVerificationResult) => {
  ...
  preflightPromotionTargetBeforeControllerStart(
    request: NativeGitPromotionTargetPreflightRequest,
  ): NativeGitPromotionTargetPreflightResult {
...
function successfulSubmoduleCheckoutSyncSummary(result: NativeGitSubmoduleCheckoutSyncResult): string | undefined {
  return result.status === "passed" ? result.summary : undefined;
}

function submoduleCheckoutSyncValidationSignals(result: NativeGitSubmoduleCheckoutSyncResult): string[] {
...
function normalizeSubmoduleUrlAgainstBase(url: string, parentBaseUrl?: string): string | undefined {
  const trimmed = url.trim();
  if (trimmed.startsWith("./") || trimmed.startsWith("../")) {
    if (!parentBaseUrl) return undefined;
...
function submoduleGitDirCandidates(repoRoot: string, submodulePath: string): string[] {
  const candidates: string[] = [];
...
interface NestedSubmoduleVerificationRequest {
  goalId: string;
  parentPath: string;
  canonicalUrl: string;
  durableRef: string;
...
interface NestedSubmoduleVerificationResult {
  changedGitlinks: ChangedSubmoduleGitlink[];
  published: PublishedSubmoduleRef[];
  verified: VerifiedSubmoduleRef[];
  blockers: SubmodulePublishBlocker[];
...
function emptyNestedSubmoduleVerificationResult(): NestedSubmoduleVerificationResult {
  return { changedGitlinks: [], published: [], verified: [], blockers: [] };
}

function verifyNestedSubmoduleGitlinks(request: NestedSubmoduleVerificationRequest): NestedSubmoduleVerificationResult {
  if (!request.policy.verifyNestedSubmodules) return emptyNestedSubmoduleVerificationResult();
  ...
  if (visited.has(visitKey)) return emptyNestedSubmoduleVerificationResult();
  ...
  try {
    ...
    if (nestedGitlinks.length === 0) return result;
    ...
    for (const nested of nestedGitlinks) {
      ...
      const fullGitlink: ChangedSubmoduleGitlink = { ...nested, path: fullPath };
...
function appendNestedVerification(target: NestedSubmoduleVerificationResult, nested: NestedSubmoduleVerificationResult): void {
...
function scanSubmoduleGitlinksInTree(repoPath: string, treeish: string): ChangedSubmoduleGitlink[] {
  ...
  for (const entry of output.split("\0")) {
    ...
    if (newSha) gitlinks.push({ path: p, status: "added", newSha });
  }
  return gitlinks;
}
