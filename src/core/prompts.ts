...
import { GOAL_QUALITY_PROFILES } from "goal-contract";

export function renderUntrustedObjectiveBlock(objective: string, label = "Goal objective"): string {
...
export function renderActiveGoalReminderPrompt(goal: GoalRecord): string {
  const remaining = goal.tokenBudget === undefined ? "unbounded" : String(Math.max(goal.tokenBudget - goal.tokensUsed, 0));
  const budgetLine =
...
export function renderCompletionAuditPrompt(request: CompletionAuditRequest): string {
...
const QUALITY_PROFILE_DISCIPLINE: Record<GoalQualityProfile, string> = {
  "incremental-implementation":
    "Execution discipline for this node:\n" +
  ...
  "test-driven-change":
    "Verification requirement for this node:\n" +
  ...
  "code-review-required":
    "Code review requirement: this node's implementation diff requires formal review before final acceptance.\n" +
    "- A dependent review/audit node will validate your changes.\n" +
    "- Do not self-certify review completeness.",
  "api-boundary-review":
    "API boundary review required: this node touches public API, events, module boundaries, or compatibility surfaces.\n" +
  ...
  "frontend-runtime-review":
    "Frontend runtime review required: this node touches browser/runtime behavior.\n" +
  ...
  "security-sensitive-review":
    "Security review required: this node touches auth, secrets, user input, data access, or external integrations.\n" +
  ...
  "performance-sensitive-review":
    "Performance review required: this node is performance/SLA-sensitive.\n" +
  ...
  "observability-required":
    "Observability required: production-visible behavior must be observable and diagnosable.\n" +
  ...
  "docs-adr-required":
    "Documentation/ADR required: architecture, API, or operational decisions require docs updates.\n" +
  ...
  "ship-preflight":
    "Ship preflight required: release-sensitive work needs launch, rollback, and monitor readiness check.\n" +
 ...
 */
export function renderQualityProfileEnvelope(node: GoalDagNode): string[] {
  ...
  for (const profile of profiles) {
    ...
    if (discipline) lines.push(discipline);
  }
