...
// ── Preflight inspector output types ──

export type GitPreflightEntryKind =
  | "root-staged"
  | "root-unstaged"
  | "root-untracked"
  | "submodule-gitlink-staged"
  | "submodule-gitlink-unstaged"
  | "submodule-internal";

export type GitPreflightEntryStatus =
  | "M"  // modified
  | "A"  // added
  | "D"  // deleted
  | "R"  // renamed
  ...
  | "T"  // type-change (file → symlink or vice-versa)
  | "X"  // unknown / other;
 ...
 */
export function inspectWorkspacePreflight(
  workspacePath: string,
  options: GitPreflightInspectorOptions = {},
): GitPreflightResult {
  ...
  const workspace: GitPreflightWorkspaceState = {
    branch,
    head,
    detached,
    headDescription,
...
function collectSubmodulePaths(
  workspacePath: string,
  repoRoot: string,
  recurse: boolean,
  maxDepth: number,
  ...
  if (!existsSync(gitmodulesPath)) return paths;
  ...
  try {
    ...
    if (recurse) {
      for (const smPath of directPaths) {
        ...
        if (!existsSync(smWorktree)) continue;
        const smGitmodules = join(smWorktree, ".gitmodules");
        if (!existsSync(smGitmodules)) continue;
        try {
          ...
          for (const nested of nestedPaths) {
            ...
            if (paths.includes(fullPath)) continue;
...
function collectSubmoduleInternalEntries(
  repoRoot: string,
  submodulePaths: string[],
  untrackedFlag: string,
): GitPreflightEntry[] {
  const entries: GitPreflightEntry[] = [];
...
function describePorcelainStatus(index: GitPreflightEntryStatus | " ", worktree: GitPreflightEntryStatus | " "): string {
  const parts: string[] = [];
  if (index !== " ") parts.push(`staged as ${index}`);
  if (worktree !== " ") parts.push(`worktree ${worktree === "?" ? "untracked" : worktree}`);
  return parts.join(", ") || "unknown";
}

function parseSubmodulePathsFromGitmodules(config: string): string[] {
  const paths: string[] = [];
  let inSection = false;
...
function parseStatusLine(line: string): Partial<GitPreflightEntry> & Pick<GitPreflightEntry, "path" | "indexStatus" | "worktreeStatus" | "xy" | "raw"> | null {
...
type SubmoduleClassification = {
  kind: "submodule-gitlink-staged" | "submodule-gitlink-unstaged" | "submodule-internal";
  submodulePath: string;
};

function classifySubmoduleEntry(
  entryPath: string,
  xy: string,
  sortedSmPaths: string[],
): SubmoduleClassification | null {
...
function porcelainCodeToStatus(code: string): GitPreflightEntryStatus | " " {
  switch (code) {
    case "M": return "M";
    case "A": return "A";
    case "D": return "D";
...
function classifyRootKind(entry: { indexStatus: GitPreflightEntryStatus | " "; worktreeStatus: GitPreflightEntryStatus | " " }): GitPreflightEntryKind {
  ...
  if (isStagedStatus(entry.indexStatus)) return "root-staged";
  if (entry.worktreeStatus === "?" || entry.indexStatus === "?") return "root-untracked";
  return "root-unstaged";
}

function isStagedStatus(code: GitPreflightEntryStatus | " "): boolean {
  return code === "M" || code === "A" || code === "D" || code === "R" || code === "C" || code === "U" || code === "T";
}

function buildRootDiagnostic(
  path: string,
  indexStatus: GitPreflightEntryStatus | " ",
  worktreeStatus: GitPreflightEntryStatus | " ",
  oldPath?: string,
): string {
  const parts: string[] = [`root worktree: ${path}`];
  if (indexStatus !== " ") parts.push(`staged as ${indexStatus}`);
  if (worktreeStatus !== " ") parts.push(`worktree has ${worktreeStatus}`);
  if (oldPath) parts.push(`(renamed from ${oldPath})`);
  return parts.join(", ");
}

function buildSubmoduleDiagnostic(classification: SubmoduleClassification): string {
...
function buildSubmoduleStatuses(
  submodulePaths: string[],
  entries: GitPreflightEntry[],
): Record<string, GitPreflightSubmoduleStatus> {
  const statuses: Record<string, GitPreflightSubmoduleStatus> = {};

  for (const smPath of submodulePaths) {
    const gitlinkEntry = entries.find(
      (e) => e.submodulePath === smPath && e.kind.startsWith("submodule-gitlink"),
    );
    const internalEntries = entries.filter(
      (e) => e.submodulePath === smPath && e.kind === "submodule-internal",
    );
...
function buildPreflightSummary(
  ...
  if (stagedRoot.length > 0) parts.push(`${stagedRoot.length} staged root change(s)`);
  if (unstagedRoot.length > 0) parts.push(`${unstagedRoot.length} unstaged root change(s)`);
  if (untrackedRoot.length > 0) parts.push(`${untrackedRoot.length} untracked root file(s)`);

  const dirtySubmodules = Object.values(submoduleStatuses).filter(
  ...
  if (dirtySubmodules.length > 0) {
    ...
    for (const sm of dirtySubmodules) {
      const smParts2: string[] = [];
      if (sm.shaMismatch) smParts2.push("gitlink changed");
      if (sm.internalDirty) smParts2.push(`${sm.internalEntries.length} internal dirty file(s)`);
 ...
 */
export function isWorkspaceDirty(workspacePath: string): boolean {
 ...
 */
export function isWorkspaceRootDirty(workspacePath: string): boolean {
 ...
 */
export function isWorkspaceSubmoduleDirty(workspacePath: string): boolean {
  const result = inspectWorkspacePreflight(workspacePath);
  const dirtySubmodules = Object.values(result.submoduleStatuses).filter(
    (s) => s.shaMismatch || s.internalDirty,
  );
