...
import {
  ...
  evaluateSubmoduleTargetBranchPolicy,
  ensureSubmoduleTargetShaAvailable,
  findGitRepositoryRoot,
  GoalRuntime,
  MemoryGoalStore,
  NativeGitWorkspaceManager,
  publishShaToSubmoduleTargetBranch,
  resolveRemoteDefaultBranch,
  resolveSubmoduleTargetBranch,
  resolveSubmoduleTargetBranchPolicy,
  TRUSTED_SUBMODULE_TARGET_BRANCH_URL_PATTERNS_ENV,
  TRUSTED_SUBMODULE_URL_PATTERNS_ENV,
  parseTrustedSubmoduleUrlPatterns,
  resolveNativeGitCloseoutPolicy,
  slugForGoal,
  slugForGoalSubagent,
  type ChangedSubmoduleGitlink,
  type GoalDagNode,
  type GoalSubagentRecord,
  type HarnessSubagentAdapter,
  type HarnessSubagentStartRequest,
  type NativeGitCloseoutPolicy,
  type NativeGitSubmoduleTargetBranchMapping,
  type NativeGitSubmoduleTargetBranchPolicy,
  type NativeGitSubmoduleTargetBranchResult,
  branchContainsCommitRemotely,
} from "../core/index.js";
...
function gitWithFileProtocol(cwd: string, args: string[]): string {
...
function createCheckoutMismatchSubmoduleFixture(): { root: string; parent: string; submodulePath: string; nextSha: string } {
...
test("native git manager rejects missing explicit controller base refs before worktree add", () => {
  ...
  try {
    ...
    assert.throws(
      () => manager.allocateControllerWorkspace({ invocationCwd: repo, goalId: "goal-missing-base", objective: "Implement missing base" }),
      /cannot resolve goal workspace base ref: feat\/missing is not a commit-ish ref/,
    );
...
test("native git auto closeout preflight blocks missing parent remote before promotion", () => {
  ...
  try {
    ...
    const preflight = manager.normalizePromotionTarget(
      { controllerWorkspacePath: controller.worktreePath, controllerBranch: controller.branch, targetRef: controller.baseRef },
      AUTO_ALLOCATED_DEFAULT_CLOSEOUT_POLICY,
    );

    assert.equal(preflight.ok, false);
    if (!preflight.ok) assert.match(preflight.reason, /cannot resolve remote URL for origin/);
    assert.equal(git(repo, ["branch", "--show-current"]), "main");
    assert.throws(() => git(repo, ["show", "HEAD:feature.txt"]));
...
test("native git promotion resolves submodule gitlink conflicts using controller objects", () => {
  const root = mkdtempSync(join(tmpdir(), "goal-promotion-submodule-conflict-"));
  ...
  try {
    ...
    git(join(parent, "aos-core"), ["config", "user.email", "goal@example.test"]);
    git(join(parent, "aos-core"), ["config", "user.name", "Goal Test"]);
    writeFileSync(join(parent, "aos-core", "target.txt"), "target\n");
    git(join(parent, "aos-core"), ["add", "target.txt"]);
    ...
    const gitlinkSha = git(parent, ["rev-parse", "HEAD:aos-core"]);
    const mergeCommit = git(join(parent, "aos-core"), ["cat-file", "-p", gitlinkSha]);
    assert.equal((mergeCommit.match(/^parent /gm) ?? []).length, 2);
    assert.equal(git(join(parent, "aos-core"), ["show", `${gitlinkSha}:target.txt`]), "target");
...
function defaultTargetBranchPolicy(overrides: Partial<NativeGitSubmoduleTargetBranchPolicy> = {}): NativeGitSubmoduleTargetBranchPolicy {
  return {
...
test("publishShaToSubmoduleTargetBranch succeeds with fast-forward publication", () => {
  ...
  try {
    const result = publishShaToSubmoduleTargetBranch(
      subWorktree,
      subRemote,
      nextSha,
      "main",
...
test("publishShaToSubmoduleTargetBranch reports alreadyContained on idempotent retry", () => {
  ...
  try {
    // First publish
    const first = publishShaToSubmoduleTargetBranch(
      subWorktree, subRemote, nextSha, "main",
      defaultTargetBranchPolicy({ trustedSubmoduleTargetBranchUrlPatterns: [subRemote] }),
    );
    assert.equal(first.published, true);

    // Second publish (idempotent retry)
    const second = publishShaToSubmoduleTargetBranch(
      subWorktree, subRemote, nextSha, "main",
      defaultTargetBranchPolicy({ trustedSubmoduleTargetBranchUrlPatterns: [subRemote] }),
    );
...
test("publishShaToSubmoduleTargetBranch blocks non-existent target branch", () => {
  ...
  try {
    const result = publishShaToSubmoduleTargetBranch(
      subWorktree, subRemote, nextSha, "nonexistent-branch",
      defaultTargetBranchPolicy({ trustedSubmoduleTargetBranchUrlPatterns: [subRemote] }),
    );
    assert.equal(result.published, false);
...
test("evaluateSubmoduleTargetBranchPolicy idempotent when SHA already contained on target branch", () => {
  ...
  try {
    ...
    const first = evaluateSubmoduleTargetBranchPolicy(
      parent,
      defaultTargetBranchPolicy({
        trustedSubmoduleTargetBranchUrlPatterns: [subRemote],
      }),
    ...
    const second = evaluateSubmoduleTargetBranchPolicy(
      parent,
      defaultTargetBranchPolicy({
        trustedSubmoduleTargetBranchUrlPatterns: [subRemote],
      }),
...
test("evaluateSubmoduleTargetBranchPolicy skips when enforcement scope is none", () => {
  const result = evaluateSubmoduleTargetBranchPolicy(
    "/tmp",
    defaultTargetBranchPolicy({ enforcementScope: "none" }),
    [{ path: "deps/sub", status: "modified", newSha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" }],
    [],
  );
