...
function createGitSubmoduleRepo(): string {
...
test("Pi goal start can load an explicit DAG file", async () => {
  ...
  try {
    goalPiExtension(pi as never);
    assert.ok(commandHandler);
    await commandHandler?.("--dag goal.trace.json", controllerCtx as never);
    assert.match(notifications.at(-1) ?? "", /Invalid goal DAG file/);
    await commandHandler?.("--dag goal.dag.json extra objective", controllerCtx as never);
...
test("goal-owned /goal start does not trigger foreground hidden continuation", async () => {
  ...
  const prompts: string[] = [];
  const sentMessages: Array<{ options: unknown }> = [];
  ...
  try {
    ...
    await commandHandler?.(
      `--workspace ${workspace} --branch main Implement orchestrated goal`,
      controllerCtx as never,
    );

    const triggerTurnCalls = sentMessages.filter(
      (entry) => (entry.options as any)?.triggerTurn === true,
    );
...
test("goal-owned /goal --dag in print mode hands off to detached controller and preserves runners on shutdown", async () => {
  ...
  const prompts: string[] = [];
  const promptOptions: Array<{ requireSessionFile?: boolean } | undefined> = [];
  const stopped: string[] = [];
