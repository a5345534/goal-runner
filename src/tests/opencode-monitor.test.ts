...
test("OpenCode sections exist: STATUS / SUMMARY / EXECUTION PLAN / RECENT EVENTS", () => {
  ...
  const completeLines = renderOpencodeMonitorLines(
    makeSummary({ status: "complete" }),
    { nodes: [makeNode({ status: "complete" })], subagents: [makeSubagent({ status: "complete" })] },
    { now: () => NOW },
  );
  ...
  const warnLines = renderOpencodeMonitorLines(
    makeSummary({ status: "complete" }),
    { nodes: [makeNode({ nodeId: "n-fail", status: "failed" })], subagents: [makeSubagent({ nodeId: "n-fail", subagentId: "sa-fail", status: "failed" })] },
    { now: () => NOW },
  );
  ...
  // Running health.
  const runningLines = renderOpencodeMonitorLines(
    makeSummary(),
    { nodes: [makeNode({ status: "running" })], subagents: [makeSubagent({ status: "running" })] },
    { now: () => NOW },
    // No ledgerEvents for running — health will be Running if running subagents exist.
  );
  ...
  const runLines = renderOpencodeMonitorLines(
    makeSummary(),
    { nodes: [makeNode({ status: "running" })], subagents: [makeSubagent({ status: "running" })] },
...
test("OpenCode runtime labels are user-facing", () => {
  ...
  const lines = renderOpencodeMonitorLines(
    makeSummary(),
    { nodes: [makeNode({ status: "running" })], subagents: [makeSubagent({ subagentId: "sa-1", status: "running" })] },
    { now: () => NOW, ledgerEvents: events, harnessState },
  );
  const joined = lines.join("\n");
...
test("OpenCode building overview with meaningful events stays within 3-8 limit", () => {
  ...
  const overview = buildGoalMonitorOverview(
    makeSummary(),
    { nodes: [makeNode()], subagents: [makeSubagent({ status: "running" })], ledgerEvents: events },
    rt,
  );
