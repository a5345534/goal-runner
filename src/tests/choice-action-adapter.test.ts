...
import {
  buildDefaultedChoiceActionResult,
  buildInteractiveChoiceActionResult,
  normalizeChoiceActionTextInput,
  renderChoiceActionTextFallback,
} from "../adapters/choice-action.js";
...
describe("normalizeChoiceActionTextInput — canonical values", () => {
  it("matches canonical value 'confirm_scope_for_analysis'", () => {
    const { result } = normalizeChoiceActionTextInput(
...
describe("buildInteractiveChoiceActionResult", () => {
  ...
  it("throws for a disabled choice when allowDisabledOverride is not set", () => {
    ...
    assert.throws(
      () => buildInteractiveChoiceActionResult(SCOPE_CONFIRM_REQUEST, abandon),
      /disabled/,
    );
  });

  it("interactive and text-alias selections produce equivalent canonical values", () => {
    const interactive = buildInteractiveChoiceActionResult(
      SCOPE_CONFIRM_REQUEST,
      SCOPE_CONFIRM_REQUEST.choices[0],
...
describe("goal-spec gate scenario equivalence", () => {
  it("Stage 1.7 scope confirmation: interactive and text produce same canonical value", () => {
    ...
    // Canonical text
    const canonical = normalizeChoiceActionTextInput(
      request,
      "confirm_scope_for_analysis",
    );
...
describe("result validation against request", () => {
  ...
  it("rejects result with mismatched canonical value", () => {
    const bogus = {
      ...
      inputMode: "interactive" as const,
      renderMode: "interactive" as const,
      selectedAt: new Date().toISOString(),
    };
    assert.throws(
      () => validateChoiceActionSelectionResultAgainstRequest(bogus, SCOPE_CONFIRM_REQUEST),
