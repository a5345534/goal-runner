...
try {
  if (args[0] === "debug") {
    ...
    if (args.length > 2) throw new Error("debug accepts at most one goal-ref");
...
async function resolveCliDefaultGoal(runtime: GoalRuntime, sessionKey: string) {
  const current = (await runtime.getGoal(sessionKey)).goal;
  if (current) return current;
  const summaries = await runtime.listGoalSummaries();
  if (summaries.length === 1) return summaries[0];
  if (summaries.length === 0) throw new Error("No current goal.");
  throw new Error("Multiple goals exist; pass debug <goal-ref>.");
}

async function resolveCliGoal(runtime: GoalRuntime, reference: string) {
  const resolved = await runtime.resolveGoalReference(reference);
  if (resolved.kind === "found") return resolved.goal;
  if (resolved.kind === "ambiguous") throw new Error(`Ambiguous goal reference ${reference}: ${resolved.matches.map((goal) => goal.shortGoalId).join(", ")}`);
